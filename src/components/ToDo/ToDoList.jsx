...
const ToDoList = () => {
    ...
    const FILTER_MAP = {
      ...
      const fillerStyles = {
        height: '100%',
        width: `${tasks.filter(item => item.done).length/ tasks.length }%`,
      ...
      return (
        <div className="todo-list">
          ...
          <progress id="file" max="100" value={`${ tasks.filter(item => item.done).length/ tasks.length*100 }`} style={labelStyles} >         </progress>
